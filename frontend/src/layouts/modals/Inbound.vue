          ...
          <v-window v-model="side" style="margin-top: 10px;">
            <v-window-item value="s">
              ...
              <v-card v-if="showLanAccessSwitch">
                <v-card-text>
                  <v-switch
                    v-model="inbound.proxy_home"
                    color="primary"
                    :label="$t('in.proxyHome')"
                    hide-details
                  />
                </v-card-text>
              </v-card>
...
<script lang="ts">
import { InTypes, ProxyHomeControlledInTypes, createInbound, Addr, ShadowTLS } from '@/types/inbounds'
...
export default {
  ...
  data() {
    return {
      ...
      ProxyHomeTypes: ProxyHomeControlledInTypes,
    }
  },
  methods: {
    ...
    changeType() {
      ...
      const prevConfig = {
        ...
        proxy_home: this.inbound.proxy_home ?? false,
      }
