    ...
    <v-col cols="12" sm="4" md="3" lg="2" v-for="(item, index) in <any[]>inbounds" :key="item.tag">
      <v-card rounded="xl" elevation="5" min-width="200" :title="item.tag">
        ...
        <v-card-text>
          ...
          <v-row v-if="proxyHomeTypes.includes(item.type)">
            <v-col>{{ $t('in.proxyHome') }}</v-col>
            <v-col>
              {{ item.proxy_home ? $t('enable') : $t('disable') }}
            </v-col>
          </v-row>
          <v-row>
...
import { ProxyHomeControlledInTypes, createInbound, Inbound } from '@/types/inbounds'
...
const proxyHomeTypes = ProxyHomeControlledInTypes
